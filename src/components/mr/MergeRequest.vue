<template>
  ...
  >
    <div class="row">
      <div class="col-lg-8">
        <a class="deco-none" :href="mergeRequest.web_url">{{ mergeRequest.title }}</a> by <strong>{{ mergeRequest.author.username }}</strong>
      </div>
      <div class="col-lg-4 text-right">
        {{ mergeRequest.created_at | dateFromNow }}
      </div>
      <div class="col-lg-12">
        <span v-for="label in mergeRequest.labels" class="badge badge-primary">{{ label }}</span>
